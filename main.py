...
def fitness(
    chromosome,
):  # Penalty for every vertex connecting 2 nodes with the same color
    fitness = 0
    for i in range(num_vertices):
    ...
    for vertex in range(len(chromosone)):
        ...
        if chromosone[vertex] in adj_colors:
            ...
            valid_colors = [
                ...
                if all_colors[i] not in adj_colors
            ]
...
def genetic_algorithm(
    ...
    population = [
        np.random.randint(0, max_colors - 1, num_vertices).tolist()
    ...
    while gen != num_generations and best_eval != 0:
        ...
        if new_best != -1:
            print(
                ">%d, new best f(%s) = %d, chromatic number = "
                % (gen, population[new_best], scores[new_best]),
                max_colors,
            )
...
print(
    f"Done! Total success rate is {success_rate*100} % with average fitness {avg_fitness}"
)
